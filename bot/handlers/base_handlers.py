...
from bot.services.db import update_prompt, erase_history, get_user_data
...
async def reset_prompt(query: CallbackQuery, state: FSMContext):
    await query.message.answer(
        'Введите новый промпт или нажмите кнопку "Отмена" внизу, для выхода в главное меню.',
...
async def get_new_prompt(message: Message, state: FSMContext):
    await update_prompt(message.from_user.id, message.text)
    await message.answer("Промт был обновлен!", reply_markup=RK.main)
    await state.clear()
...
async def get_info(message: Message) -> None:
    await message.answer(
        'Здесь всякая инфа. \n\
Вопросы? Пиши <a href="t.me/ceo_of_seks">мне</a>\
', parse_mode=ParseMode.HTML)
...
async def cancel_state(message: Message, state: FSMContext):
    await message.answer("Действие отменено.", reply_markup=RK.main)
    await state.clear()


async def any_message(message: Message) -> None:
    answer = await chat_completion(message)
